...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int n : TAMANHOS) {
            ...
            rodar("1. Inserção Pura",
                    TestOnlyInsert.executarInsert(n, WARMUP, REPETICOES));

            rodar("2. Remoção Pura",
                    TestOnlyRemove.executarRemove(n, WARMUP, REPETICOES));

            rodar("3. Peek Puro",
                    TestOnlyPeek.executarPeek(n, WARMUP, REPETICOES, REPETICOES_PEEK));

            rodar("4. Misto (Insert + Remove)",
                    TestBothInsertRemove.executarMisto(n, WARMUP, REPETICOES));

            rodar("5. Carga Pesada (50% insert / 30% remove / 20% peek)",
                    TestHeavyLoad.executeHeavyLoad(n, WARMUP, REPETICOES));
        }
    ...
    private static void imprimirTempo(TestResults r) {
        ...
        System.out.printf("    %-12s  média: %10.3f ms  ± %.3f ms%n",
                "PQHeap",   mediaHeap, dpHeap);
        System.out.printf("    %-12s  média: %10.3f ms  ± %.3f ms%n",
                "PQTreeMap", mediaTree, dpTree);
    }
    ...
    private static void imprimirMemoria(TestResults r) {
        ...
        System.out.printf("    %-12s  média: %,14.0f bytes  ± %,.0f%n",
                "PQHeap",    mediaHeap, dpHeap);
        System.out.printf("    %-12s  média: %,14.0f bytes  ± %,.0f%n",
                "PQTreeMap", mediaTree, dpTree);
    }
    ...
    private static void imprimirVencedor(TestResults r) {
        ...
        if (mH < mT) {
            vencedor   = "PQHeap";
            percentual = (mT - mH) / mT * 100.0;
        } else if (mT < mH) {
            vencedor   = "PQTreeMap";
            percentual = (mH - mT) / mH * 100.0;
        } else {
            System.out.printf("  ► Empate técnico%n");
            return;
        }

        System.out.printf("  ► %s foi mais rápido(a) em %.1f%%%n", vencedor, percentual);
    }
